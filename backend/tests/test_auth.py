...
async def _create_user(email: str, password: str, role: UserRole = UserRole.jefe) -> None:
    async with AsyncSessionLocal() as db:
        db.add(User(email=email, rut=fake_rut(email), full_name="Usuario de prueba", role=role,
...
async def test_login_success_and_me(client):
    ...
    res = await client.post("/api/v1/auth/login", data={"username": fake_rut("jefe@test.com"), "password": "Clave123!"})
...
async def test_login_wrong_password(client):
    ...
    res = await client.post("/api/v1/auth/login", data={"username": fake_rut("mecanico@test.com"), "password": "incorrecta"})
    assert res.status_code == 401


async def test_login_unknown_user(client):
    res = await client.post("/api/v1/auth/login", data={"username": fake_rut("nadie@test.com"), "password": "loquesea"})
    assert res.status_code == 401
...
async def test_refresh_rotates_and_old_token_stops_working(client):
    ...
    login = await client.post("/api/v1/auth/login", data={"username": fake_rut("encargado@test.com"), "password": "Clave123!"})
...
async def test_logout_revokes_access_token(client):
    ...
    login = await client.post("/api/v1/auth/login", data={"username": fake_rut("logout@test.com"), "password": "Clave123!"})
...
async def test_login_rate_limit_blocks_after_too_many_attempts(client):
    for _ in range(5):
        await client.post("/api/v1/auth/login", data={"username": fake_rut("bloqueado@test.com"), "password": "mal"})

    blocked = await client.post("/api/v1/auth/login", data={"username": fake_rut("bloqueado@test.com"), "password": "mal"})
